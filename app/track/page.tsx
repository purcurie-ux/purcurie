...
export default function TrackOrderPage() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/track?waybill=${waybill}`);
          ...
          return (
            <div key={index} style={{ display: "flex", gap: "20px", position: "relative" }}>
              ...
              <div style={{ paddingBottom: "40px", flex: 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  {isActive && index !== 5 && (
                    <button 
                      ...
                      style={{ color: activeColor, fontSize: "0.95rem", fontWeight: "600", background: "none", border: "none", cursor: "pointer", padding: 0 }}
                    >
                      More info
                    </button>
                  )}
                ...
                {step.date && (
                  <p style={{ margin: "4px 0 0 0", color: isFuture ? "#9CA3AF" : "#6B7280", fontSize: "0.9rem" }}>
                    {step.date}
                  </p>
                )}
  ...
  // Sort scans chronologically (oldest at top, newest at bottom) for the modal
  const getSortedScans = () => {
    if (!trackingData || !trackingData.Scans) return [];
    return [...trackingData.Scans].sort((a: any, b: any) => {
      return new Date(a.ScanDetail.ScanDateTime).getTime() - new Date(b.ScanDetail.ScanDateTime).getTime();
    });
  };
    ...
    <div style={{ maxWidth: "800px", margin: "80px auto", padding: "0 20px", minHeight: "60vh" }}>
      <div style={{ textAlign: "center", marginBottom: "40px" }}>
        ...
        <p style={{ color: "#666", fontSize: "1.1rem" }}>
          Enter your Delhivery AWB number for real-time delivery updates.
        </p>
      </div>

      <form onSubmit={handleTrack} style={{ display: "flex", gap: "10px", marginBottom: "40px", maxWidth: "500px", margin: "0 auto 40px auto" }}>
        <input
          type="text"
      ...
      {error && (
        <div style={{ maxWidth: "500px", margin: "0 auto 20px auto", padding: "15px", backgroundColor: "#fff4f4", color: "#8e1919", borderRadius: "8px", borderLeft: "4px solid #8e1919" }}>
          {error}
        </div>
      )}
      ...
      {/* ✅ POPUP MODAL FOR TRANSIT HISTORY */}
      {showDetails && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: "24px", borderBottom: "1px solid #E5E7EB", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <h2 style={{ margin: 0, fontSize: "1.75rem", fontWeight: "bold", color: "#374151" }}>Transit History</h2>
                <p style={{ margin: "6px 0 0 0", color: "#6B7280", fontSize: "0.95rem" }}>All tracking history details</p>
              </div>
              <button 
            ...
            <div style={{ padding: "30px 24px", overflowY: "auto", flex: 1 }}>
              <div style={{ display: "flex", flexDirection: "column" }}>
                {getSortedScans().map((scanObj: any, i: number, arr: any[]) => {
                  const isLast = i === arr.length - 1; // The active/latest status
                  ...
                  return (
                    <div key={i} style={{ display: "flex", gap: "20px", position: "relative" }}>
                      ...
                      {!isLast && (
                        <div style={{ position: "absolute", left: "11px", top: "30px", bottom: "-5px", width: "2px", backgroundColor: activeColor, zIndex: 0 }} />
                      )}
                      
                      {/* Timeline Dot / Icon */}
                      <div style={{ 
                        width: "24px", height: "24px", borderRadius: "50%", 
                        backgroundColor: "#fff", border: `2px solid ${activeColor}`, 
                        display: "flex", alignItems: "center", justifyContent: "center", 
                        color: activeColor, zIndex: 1, marginTop: "2px", flexShrink: 0
                      }}>
                      ...
                      <div style={{ paddingBottom: "35px", flex: 1 }}>
                        <div style={{ fontWeight: 600, color: isLast ? activeColor : "#374151", fontSize: "0.95rem" }}>
                          {formatDate(scan.ScanDateTime)}
                        </div>
                        <div style={{ color: isLast ? activeColor : "#6B7280", fontSize: "0.9rem", marginTop: "4px", lineHeight: "1.4" }}>
                          {scan.Scan} {scan.ScannedLocation ? `at ${scan.ScannedLocation}` : ""}
                        </div>
                      </div>
                    </div>
                  );
